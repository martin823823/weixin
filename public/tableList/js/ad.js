...
var Effect = (function() {
    var Slider = function(o) {
        ...
        this.touchAd      = this.setting.touchAd || false;;
        this.timer        = null;
...
function init(){
    var a = 1.9;
    var bannerPic = $("#slider").width();
    if(bannerPic){
        var bannerPic = $("#slider");
        var bannerWidth = bannerPic.parent().width();
        bannerPic.find('li').width(bannerWidth);
        bannerPic.height(bannerPic.width()/a);
        Effect.slider({
            'target': 'slider',
            'showMarkers': true,
            'touchAd': true
...
$(function(){
    // 调用语句
    init();
    $('#slider ol').css('margin-left','-'+$('#slider ol').width()/2+'px');
});